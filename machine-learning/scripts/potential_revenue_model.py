"""
CP2 – Per Unit Price Multiplier Model, portable artifact
Predicts a per unit price multiplier m in [0.85, 1.10].
ETL usage:
  ExpectedPerUnitPrice = TCP_Model * m_pred
  PotentialRevenue = PlannedUnits * ExpectedPerUnitPrice * AbsorptionRate
"""
...
# -----------------------------------------------------------
# DEFENSIBLE PSEUDO LABEL FOR MULTIPLIER
# -----------------------------------------------------------
base = 0.95
m = (
    base
...
if n_splits == 1:
    ...
    for tr, te in gkf.split(X, y_mult, groups=groups):
        ...
        print(f"Fold {fold}: [mult] R²={r2_m:.4f}, MAE={mae_m:.4f}, RMSE={rmse_m:.4f} "
...
rel_mae_p  = mae_p_all / max(np.mean(y_true_p_all), 1.0)

print("\n=== CV Summary ===")
print(f"[mult]  R²={r2_m_all:.4f}, MAE={mae_m_all:.4f}, RMSE={rmse_m_all:.4f}")
print(f"[price] R²={r2_p_all:.4f}, MAE={mae_p_all:,.0f}, RMSE={rmse_p_all:,.0f}, Rel MAE={rel_mae_p*100:.2f}%")

# -----------------------------------------------------------
# FINAL FIT AND SAVE, portable
# -----------------------------------------------------------
pipe.fit(X, y_mult)
...
print(f"\n✅ Saved model  → {MODEL_OUT_JOBLIB.name} and {MODEL_OUT_PKL.name}")
print(f"✅ Saved meta   → {META_OUT.name}")

# -----------------------------------------------------------
# Sensitivity, monotonic sanity
# -----------------------------------------------------------
X_sens = X.copy()
